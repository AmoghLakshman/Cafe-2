...
# ============================================================================
    ...
    tab1, tab2, tab3 = st.tabs(["💰 Pricing Engine", "🧬 Persona Matcher", "🔗 Menu Recommender"])
    
    # --- TAB 1: REGRESSION SIMULATOR (Pricing) ---
    with tab1:
        st.header("💰 The Spending Predictor")
        st.markdown("Predicts exactly **how much** a customer will spend per visit based on their profile (Powered by Lasso Model).")
        
        col1, col2 = st.columns(2)
        with col1:
            # Regression inputs based on our Lasso coefficients
            reg_income = st.selectbox("Income Bracket", 
                ["Less than 5,000", "5,000 - 10,000", "10,001 - 20,000", "20,001 - 35,000", "35,001 - 50,000", "50,001 - 75,000", "Above 75,000"],
                index=3)
        ...
        # Calculate Predicted Spend (Simplified Lasso Logic)
        # Base Spend ~35 AED
        base_spend = 35.0
        
        # Income Impact
        if reg_income == "Above 75,000": base_spend += 117.24
        elif reg_income == "50,001 - 75,000": base_spend += 89.74
        elif reg_income == "35,001 - 50,000": base_spend += 14.16
        elif reg_income == "Less than 5,000": base_spend -= 46.20
        elif reg_income == "5,000 - 10,000": base_spend -= 39.10
        
        # Reason Impact
        if "Food quality" in reg_reason and "Work/study" in reg_reason: base_spend += 26.42
        if "Coffee quality" in reg_reason and "Food quality" in reg_reason: base_spend -= 11.61
        ...
        with col2:
            ...
            if final_spend > 100:
                st.success("🚀 **High Value Customer!** Target with premium membership.")
            elif final_spend > 50:
                st.info("⚖️ **Mid Value Customer.** Target with loyalty cards.")
            else:
                st.warning("⚠️ **Budget Customer.** Target with daily combos.")

    # --- TAB 2: CLUSTERING SIMULATOR (Personas) ---
    with tab2:
        st.header("🧬 The Persona Matcher")
        st.markdown("Classify a new customer into one of our 4 DNA segments (Powered by K-Means Centroids).")
        ...
        for name, center in centroids.items():
            dist = np.linalg.norm(user_point - center)
            if dist < min_dist:
                min_dist = dist
                best_cluster = name
        ...
        with col2:
            ...
            if "Premium" in best_cluster:
                st.success("⭐ **VIP Target!** Sell them the Elite Membership immediately.")
                st.balloons()
            elif "Bookworm" in best_cluster:
                st.success("📚 **Loyal Reader.** Sell them the Book Club Membership.")
            elif "Social" in best_cluster:
                st.warning("💰 **High Spender, No Membership.** Upsell food & events.")
            else:
                st.error("💸 **Low Value.** Stick to basic offers.")

    # --- TAB 3: ARM SIMULATOR (Bundles) ---
    with tab3:
        st.header("🔗 The Smart Menu Recommender")
        st.markdown("Recommend the perfect product bundle based on customer interest (Powered by Apriori Rules).")
        ...
        if interest == "Business Books":
            ...
            st.markdown("**Why?** Our data shows these items are bought together with **2.89x Lift**.")
            ...
            st.markdown("**Why?** Strong association (**2.03x Lift**) between fiction readers and light snacks.")
            ...
            st.secondary("✅ **Recommendation:** 'The Study Pass'")
